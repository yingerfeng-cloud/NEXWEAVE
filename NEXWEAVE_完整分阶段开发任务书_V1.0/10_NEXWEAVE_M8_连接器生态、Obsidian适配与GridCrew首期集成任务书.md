# NEXWEAVE M8：连接器生态、Obsidian适配与GridCrew首期集成任务书
...
- ConnectorDefinition/Instance/CredentialRef/SyncRun/Watermark。
- 连接器不得绕过Source、权限、密级、审计和幂等。
- 提供SDK、Mock Server、契约测试和错误语义。
### Obsidian

- Markdown/YAML导出、Vault目录映射、稳定ID和反向导入冲突检测。
...
7. 不得将NEXWEAVE代码合并到GridCrew内核。
8. GridCrew不得调用草稿查询或直接写正式知识。
