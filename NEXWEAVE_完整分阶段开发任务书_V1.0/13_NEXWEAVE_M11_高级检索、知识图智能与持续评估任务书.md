# NEXWEAVE M11：高级检索、知识图智能与持续评估任务书
...
提升大规模知识下的召回、图遍历、增量编译、模型选择和质量诊断能力，但保持证据与人工审核边界。
...
1. 适配OpenSearch/Milvus/Neo4j或NebulaGraph Provider，保持可替换。
2. 实现分层/多向量检索、混合排序、查询规划和关系路径约束。
3. 实现持续增量编译、来源变更检测、知识新鲜度和影响分析。
4. 实现模型/Prompt/检索策略评测与自动回归。
...
- 标题、正文、属性、案例、Claim、Evidence多索引；按权限预过滤。
- RRF/学习排序可选，但必须提供来源与排名解释，不将分数称为事实概率。
### 图智能

- 图数据库适配、路径约束、时间图、冲突图和影响分析。
- 图数据库是查询投影，不成为绕过数据库与Release的新权威源。
### 持续编译

- Source变更检测、增量影响范围、过期知识标记、重新审核和补丁Release。
### 评测运营

- Evaluation Registry、基准集版本、离线评测、在线反馈、回归阻断和漂移监控。
...
- [ ] 切换Provider不改变核心领域契约和Release语义。
- [ ] 增量更新仅影响可解释范围，旧Release仍可复现。
- [ ] 评测下降可自动阻断发布。
- [ ] 全局CI门禁通过；
- [ ] 无新增P0安全、架构、证据或版本问题；
- [ ] 用户已有修改未被覆盖；
