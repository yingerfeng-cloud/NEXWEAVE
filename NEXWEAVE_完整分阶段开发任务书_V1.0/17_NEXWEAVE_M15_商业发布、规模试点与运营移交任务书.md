# NEXWEAVE M15：商业发布、规模试点与运营移交任务书
...
完成产品级发布、多个领域/客户试点、运营体系和完整交付，使平台具备持续售卖与维护能力。

---

## 2. 前置条件

- M14多应用闭环通过，产品范围与商业版本明确。
...
1. 完成Release Notes、License、版本支持策略和升级路径。
2. 完成至少两个领域包或两个业务应用的规模试点，验证平台通用性。
3. 建立SLA、监控、告警、工单、问题分级、版本维护和客户成功指标。
4. 完成安全、性能、灾备、合规、文档和运维移交验收。
...
- 语义版本、支持周期、补丁策略、兼容矩阵、弃用策略和升级工具。
- 安装包、离线包、演示包、管理员手册、开发者文档和用户培训。
### 规模试点

- 不同领域Pack、不同部署环境、不同IAM/数据库的真实运行。
- 30天或约定周期的活跃使用、知识质量、专家接受率和业务价值指标。
### 运营体系

- SLO/SLA、告警、容量、成本、质量趋势、客户工单和漏洞响应。
- 建立RCA、问题复盘、变更委员会和版本发布机制。
### 最终验收

- 全量需求追踪、架构债务清零/豁免、渗透与安全扫描、恢复演练、E2E。
...
- NEXWEAVE正式产品版本、完整交付包、试点与验收报告、运营移交。
...
- [ ] 所有R1—R3需求有完成、延期或明确取消结论。
- [ ] 至少两个领域/应用证明平台核心无场景硬编码。
- [ ] 运维团队可独立部署、恢复、升级和处理常见故障。
- [ ] 安全、质量和Release审计可供外部检查。
- [ ] 全局CI门禁通过；
- [ ] 无新增P0安全、架构、证据或版本问题；
- [ ] 用户已有修改未被覆盖；
- [ ] 执行回报与真实代码、测试结果一致。
