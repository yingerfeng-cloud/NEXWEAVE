...
- 正式 M0/R1 仍须按原任务书使用真实 PostgreSQL、MinIO、Redis、Temporal 等依赖完成集成和 E2E 验收。
...
1. `domain` 和 `contracts` 不得依赖 FastAPI、SQLAlchemy、SQLite、Temporal、Redis、MinIO 或模型厂商 SDK。
...
1. 不依赖 Docker、WSL、PostgreSQL、Redis、MinIO 或 Temporal 安装。
...
- Provider 契约测试；
- PostgreSQL、MinIO、Redis、Temporal 适配设计；
...
- `integration-real`：后续连接 PostgreSQL/MinIO/Redis/Temporal；
...
- [ ] 已明确列出与真实 PostgreSQL、MinIO、Redis、Temporal 的差距。
