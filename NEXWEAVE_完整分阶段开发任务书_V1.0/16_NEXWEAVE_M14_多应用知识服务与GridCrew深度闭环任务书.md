# NEXWEAVE M14：多应用知识服务与GridCrew深度闭环任务书
...
把NEXWEAVE建设为多业务应用共用的可信知识服务，并完成GridCrew任务结果回流、审核和再发布闭环。
...
1. 实现多应用Client、用途策略、版本订阅、查询SLA和使用审计。
2. 实现GridCrew反馈/案例草稿→NEXWEAVE编译→专家审核→新Release→Skill升级。
3. 实现Webhook/Event订阅、兼容性通知、灰度升级和回放。
4. 提供Java/Python/TypeScript SDK和参考应用。
...
- ApplicationClient、UseCasePolicy、ReleaseSubscription、Quota和SLA。
- 不同应用对草稿、原文、Evidence和高风险知识的访问策略。
### GridCrew闭环

- 任务、证据、人工结论作为Source或Feedback草稿进入，不直接写正式知识。
- 知识发布触发Skill版本候选，需GridCrew侧审批后升级。
- 跨平台correlation_id、身份、审计、幂等和失败补偿。
### 事件与兼容

- 发布/废止/冲突/Pack升级事件，Schema版本与SDK兼容。
- 消费者失败重试、死信、回放和订阅状态。
### 参考应用

- 可信问答、RCA知识助手、合同知识或消防知识示例，验证通用性。
...
- [ ] GridCrew完成“任务→案例草稿→审核→Release→Skill升级”全链路。
- [ ] 任何回流内容未经审核不能进入正式查询。
- [ ] 多应用版本订阅互不影响且可回滚。
- [ ] 全局CI门禁通过；
- [ ] 无新增P0安全、架构、证据或版本问题；
...
7. 不得让业务应用直接修改NEXWEAVE数据库。
8. 不得自动升级生产Skill而无审批和兼容检查。
