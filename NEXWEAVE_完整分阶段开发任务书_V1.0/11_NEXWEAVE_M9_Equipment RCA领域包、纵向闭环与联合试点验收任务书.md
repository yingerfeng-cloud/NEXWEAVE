...
使用真实但脱敏的设备RCA资料验证平台通用性和业务价值，不把知识问答误称为全自动根因诊断。

---

## 2. 前置条件

- M8平台、Release、查询和GridCrew只读集成可用。
- 已获得合规、脱敏、可用于测试的RCA报告和专家资源。
...
2. 导入历史RCA/IOE/LOE/设备手册，形成可审核的设备知识。
3. 验证相似案例、故障机理路径、支持/反向证据和验证方法问答。
4. 在GridCrew中演示RCA数字员工调用固定知识版本。
5. 完成真实E2E、专家评审和R1联合试点验收。
...
- GridCrew场景完成任务发起、知识调用、引用展示和反馈草稿提交。
### E2E与验收

- 真实Temporal、真实数据库、真实对象存储、至少一种真实模型或内网模型。
- 故障演练：模型超时、解析失败、审核退回、发布失败、回滚、GridCrew重试。
...
- Equipment RCA Pack V1.0、RCA知识Release、标准问题集和试点报告。
- GridCrew联动Demo、完整E2E与故障演练报告。
