# NEXWEAVE M12：高可用、灾备、国产化与规模性能任务书
...
完成企业私有化生产部署、容灾、性能、离线环境和国产技术栈适配，形成R2生产版。

---

## 2. 前置条件

- M11功能稳定，容量模型和目标部署环境明确。
...
1. 完成Kubernetes部署、弹性Worker、HA、备份恢复和灾难演练。
2. 验证百万实体、数百万关系和并发查询/编译性能。
3. 建立PostgreSQL/达梦、RustFS/国产对象存储、国产中间件和浏览器适配层。
4. 形成SBOM、漏洞、许可证、镜像签名和离线安装包。
...
- API/Worker/Temporal/PostgreSQL/对象存储高可用方案。
- RPO/RTO目标、备份校验、跨环境恢复、Release和Raw恢复演练。
### 性能

- 容量模型、压测数据生成、查询P95/P99、编译吞吐、队列积压和资源隔离。
- 慢查询、索引、分区、归档和冷热分层。
### 国产化

- 数据库方言与迁移适配、SQL兼容测试、OpenJDK/Java壳仅作为交付适配。
- 麒麟、国产浏览器、国产中间件和离线模型服务验证。
### 供应链安全

- SBOM、依赖许可证、镜像签名、漏洞扫描、密钥轮换、离线制品仓库。
...
- [ ] 达到PRD约定的页面、检索、任务进度和可用性目标，或记录经批准的偏差。
- [ ] 灾备恢复能够还原Raw、Release、审计和任务状态。
- [ ] 国产化适配不产生第二套领域内核。
- [ ] 全局CI门禁通过；
- [ ] 无新增P0安全、架构、证据或版本问题；
- [ ] 用户已有修改未被覆盖；
