# NEXWEAVE M7：知识质量、不可变发布、图谱检索与可信问答任务书
...
> 执行状态：用户于 2026-08-31 正式下发并正式验收；ADR-0026、实现和本地技术验收已完成；停止在已验收 M7，不得进入 M8。

---

## 1. 阶段定位

完成“审核知识→质量门禁→正式Release→业务查询”的闭环，使业务应用只消费可复现版本。
...
1. 实现Lint、标准问题集、回归评估和发布门禁。
2. 实现ReleaseCandidate、审批、不可变Release、灰度、回滚与订阅。
3. 实现关键词、属性、语义、关系遍历和混合检索。
4. 实现关系图谱、Query API和Ask NEXWEAVE的带引用可信回答。
...
- Lint：缺字段、无来源、无效引用、重复实体、孤立页面、冲突、过期知识和关系方向。
- EvaluationSuite 支持可回答、不可回答、反事实、冲突、多来源和证据不足问题。
- A/B比较不同模型、Prompt和检索策略；记录问题级错误。
### 发布管理

- ReleaseItem 固化页面、实体、关系、Claim、Evidence、Schema、Prompt/索引版本。
- 发布验证、审批、部署、回滚、废止和历史版本查询。
- Release后不可修改，回滚切换服务指针而不篡改历史。
### 检索与图谱

- Search Provider、Vector Provider、Graph Query Port；MVP可用PostgreSQL/pgvector/Relation表。
- 混合召回、元数据过滤、权限过滤、RRF或可解释融合排序。
- 一跳/多跳、最短路径、因果路径、时间切片和关系证据查看。
### 可信问答

- 默认只查询指定正式Release；回答记录QuerySession/Answer/Citation。
- 输出直接回答、关键依据、原始引用、实体关系、不确定性、冲突和版本。
- 证据不足必须拒绝下结论；引用必须通过Anchor校验。
### 前端

- 质量中心、发布管理、关系图谱、Ask NEXWEAVE完整页面。
...
- 质量评估、Release服务、查询与检索服务、图谱服务、Ask NEXWEAVE。
- 发布/回滚/回答复现报告。
...
- [ ] 从资料上传到正式Release形成完整真实闭环。
- [ ] 发布知识来源可追溯率与Schema合规率达到100%。
- [ ] 同一Query在固定Release和配置下可复现关键引用。
- [ ] 切换/回滚Release不会修改历史对象；草稿默认不可查询。
- [ ] 全局CI门禁通过；
- [ ] 无新增P0安全、架构、证据或版本问题；
- [ ] 用户已有修改未被覆盖；
- [ ] 执行回报与真实代码、测试结果一致。
