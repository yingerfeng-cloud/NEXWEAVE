# NEXWEAVE M6：主张证据、冲突处理与专家审核闭环任务书
...
把AI候选知识转化为可核对、可分派、可审计的专家审核对象，建立正式知识前的责任闭环。

---

## 2. 前置条件

- M5 可稳定生成草稿和候选Evidence。
...
1. 实现 Claim/Evidence 的标准服务、支持证据与反向证据。
2. 实现重复、数值、时间、关系、版本和来源权威冲突。
3. 实现知识工程师初审、领域专家复核、负责人批准的可配置审核流程。
4. 实现职责分离、超时、补充资料、批量低风险审核和全量审计。
...
- 支持 Subject-Predicate-Object、Scope、有效期、可信等级、支持/反对方向。
- 因果 Relation 与正式 Claim 的 Evidence 强约束；证据侧栏可跳原文。
- Evidence 的定位、来源等级、审核状态、截图/锚点和失效检测。
### 冲突中心

- 冲突检测规则、聚类、双方证据对比、来源权威等级和建议动作。
- 支持保留A/B、条件化共存、合并、未决、过期与重新打开。
- 未解决阻断项必须可阻断Release。
### 审核中心

- HumanReviewWorkflow：分派、领取、逐项接受/修改/驳回、补充资料、转交、批准。
- 高风险知识创建人与批准人分离；所有修改保存diff和理由。
- 审核队列按风险、来源、类型、时限和空间筛选。
### 质量抽检

- 建立审核一致性抽检、专家修改比例、接受率和争议统计。
...
- Claim/Evidence中心、冲突中心、审核中心和审批策略引擎。
- 审核记录、职责分离与质量统计。
...
- [ ] 没有Evidence的正式Claim或因果Relation无法通过审核。
- [ ] 冲突处理保留双方原始证据和完整决策记录。
- [ ] 审核任务可超时升级、补充资料后恢复，并可从Temporal重建。
- [ ] 高风险知识不能由同一人创建并最终批准。
- [ ] 全局CI门禁通过；
- [ ] 无新增P0安全、架构、证据或版本问题；
- [ ] 用户已有修改未被覆盖；
