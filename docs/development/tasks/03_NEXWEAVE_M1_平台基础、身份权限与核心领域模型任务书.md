# NEXWEAVE M1：平台基础、身份权限与核心领域模型任务书
...
构建所有后续知识能力依赖的平台底座，形成生产兼容的租户、空间、身份、权限、存储、审计和核心对象持久化。

---

## 2. 前置条件

- M0 门禁通过，核心契约和数据库策略已冻结。
...
1. 实现 Tenant/Organization/User/ServiceIdentity 与 KnowledgeSpace 基础。
2. 实现 OIDC 兼容认证、RBAC+ABAC 授权和空间隔离。
3. 实现对象存储、文件元数据、checksum、审计、Outbox 和系统配置。
4. 实现平台前端壳、设计系统、导航、空间切换与系统管理基础。
...
- 实现本地开发身份提供者适配和 OIDC Provider 接口；生产不锁定单一 IAM。
- 实现平台角色、空间角色、资源权限、对象权限和密级属性检查。
- 实现服务身份、令牌受众、权限拒绝审计与最小权限默认策略。
### 核心领域与数据库

- 实现 KnowledgeSpace、SpaceMember、ModelProfile、PromptVersion、ConnectorDefinition、AuditLog、OutboxEvent。
- 所有业务表带 tenant_id、space_id、created_by、updated_by、created_at、updated_at、version/etag 等统一字段。
- 实现乐观锁、软归档与不可物理删除策略的通用基础设施。
### 文件与对象存储

- 实现 ObjectStorage Port 与 RustFS S3 Adapter；对象 key、checksum、content_type、size、classification 可追踪，业务层不得依赖厂商 SDK 或管理 API。
- 实现受控上传会话、预签名 URL 或代理上传策略；下载必须经过权限校验。
- 建立病毒扫描/恶意文件扫描扩展点，M1 可使用 Stub 但不得绕过状态。
### 前端与管理

- 实现登录态、应用框架、16个导航占位路由、空间切换、权限守卫、全局错误处理。
- 实现知识空间创建/编辑/归档基础页面；不包含资料编译功能。
- 实现用户、角色、模型配置、审计查询的基础管理页面。
### 可观测性

- 接入 OpenTelemetry trace/log/metric，统一 trace_id 贯穿 Web/API/DB/Object Storage。
- 实现健康、就绪、版本和配置诊断端点。
...
- 身份与权限服务、知识空间服务、对象存储基础、审计与 Outbox。
- 平台前端框架和空间管理闭环。
- 数据库迁移、权限矩阵、OpenAPI 和 SDK 基础。
...
- [ ] 跨租户和跨空间访问被服务端阻断，并留下审计记录。
- [ ] 上传对象具有 checksum 且不可被同 key 静默覆盖。
- [ ] 空间创建、成员授权、权限撤销和归档形成真实前后端闭环。
- [ ] 所有核心操作具备 trace_id 和审计字段。
- [ ] 全局CI门禁通过；
- [ ] 无新增P0安全、架构、证据或版本问题；
- [ ] 用户已有修改未被覆盖；
- [ ] 执行回报与真实代码、测试结果一致。
