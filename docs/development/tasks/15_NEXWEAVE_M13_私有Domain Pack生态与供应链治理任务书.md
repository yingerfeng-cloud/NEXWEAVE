# NEXWEAVE M13：私有Domain Pack生态与供应链治理任务书
...
将领域包从研发配置提升为企业可治理、可签名、可分发、可升级的生态资产。
...
1. 实现官方/企业/第三方Pack注册中心和私有仓库。
2. 实现签名、来源、许可证、依赖、兼容、漏洞和审批治理。
3. 实现Pack开发者工具、模板、测试沙箱、认证与发布流程。
...
- 版本、渠道、依赖、平台兼容、签名、撤回、废止和安全公告。
- 企业私有Pack隔离和组织级分发策略。
### 开发者工具

- CLI、模板、Schema/Prompt/Lint/Evaluation测试、样例生成和兼容检查。
- 沙箱禁止任意代码和越权数据访问。
### 供应链治理

- Pack来源验证、SBOM、许可证、恶意Prompt/模板扫描和人工审核。
### 运营

- 安装量、升级率、兼容风险、质量指标和反馈。
...
- 私有Pack Registry、开发者SDK/CLI、供应链治理流程。
...
- [ ] Pack可独立开发、测试、签名、发布、安装、升级、回滚和撤回。
- [ ] 不可信Pack无法访问平台内部实现或绕过权限。
- [ ] 全局CI门禁通过；
- [ ] 无新增P0安全、架构、证据或版本问题；
- [ ] 用户已有修改未被覆盖；
- [ ] 执行回报与真实代码、测试结果一致。
