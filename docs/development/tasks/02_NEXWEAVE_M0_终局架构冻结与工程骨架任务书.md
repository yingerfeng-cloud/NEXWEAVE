...
> 2026-08-24 批准修订：对象存储实现统一替换为 RustFS/S3，不保留旧 Provider 回退；见 ADR-0017。原始交付包保持原文用于审计。
...
- 建立 Docker Compose：PostgreSQL、RustFS、Redis、Temporal、API、Web 最小服务；暂不要求 OpenSearch/Neo4j。
### 契约与数据
...
- [ ] 一条命令可启动基础环境并通过 API/Web/Temporal/DB/RustFS 健康检查。
- [ ] 领域包与平台内核依赖方向有自动化门禁。
